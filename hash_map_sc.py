...
class HashMap:
    ...
    def put(self, key: str, value: object) -> None:
        """
        This method updates a key/value pair in the hash map. If the given key already exists in the
        hash map, its associated value must be replaced with the new value. If the given key is not
        in the hash map, a new key/value pair must be added.

        If the current load factor is greater than or equal to 1 the array is resized to doulbe its
        current capacity.
        """
    ...
    def empty_buckets(self) -> int:
        """
        return the number of empty buckts in the hash table
        """
        empty = 0
